package com.coderedrobotics.uri.statics;

/**
 *
 * @author Michael
 */
public class Wiring {
    /*
     * MOTOR CONTROLLERS 
     */
    
    public static final int FRONT_LEFT_MOTOR = 2;
    public static final int FRONT_RIGHT_MOTOR = 1;
    public static final int REAR_LEFT_MOTOR = 3;
    public static final int REAR_RIGHT_MOTOR = 0;
    public static final int LIFT_MOTOR = 4;
    
    /*
     * DIO 
     */
    
    public static final int FRONT_LEFT_ENCODER_A = 0;
    public static final int FRONT_LEFT_ENCODER_B = 1;
    public static final int FRONT_RIGHT_ENCODER_A = 2;
    public static final int FRONT_RIGHT_ENCODER_B = 3;
    public static final int REAR_LEFT_ENCODER_A = 4;
    public static final int REAR_LEFT_ENCODER_B = 5;
    public static final int REAR_RIGHT_ENCODER_A = 6;
    public static final int REAR_RIGHT_ENCODER_B = 7;
    
    /*
     * ANALOG INPUTS
     */
    
    /*
     * RELAYS 
     */
    public static final int RED_AND_GREEN_LEDS = 0;
    public static final int BLUE_LEDS = 1;
    
    /*
     * PNUEMATICS
     */
}
